...
Aggregation groups by `product_id` across the child orders.
...
- output **only full cartons** (remainder is NOT included in the Mega order)

### Remainder Handling (Priority-based Cuts)

Since only full cartons can be purchased, the remainder units are "cut" from individual orders.

**Priority rules:**

- Orders are sorted by `created_at` timestamp
- **Oldest orders have highest priority** (they keep their items)
- **Newest orders have lowest priority** (they get cut first)

**Example:**

- Product X has `package_quantity = 12` (12 units per carton)
- Alice ordered 5 units at 10:00 AM
- Bob ordered 4 units at 11:00 AM
- Carol ordered 5 units at 12:00 PM
- Total: 14 units → 1 carton (12 units) + 2 remainder

Cuts are distributed starting from the newest order:

1. Carol loses 2 units (her 5 → 3 effective)
2. Bob keeps all 4 units
3. Alice keeps all 5 units

The `cuts` array in the API response shows exactly what was cut from each order.

### For unit products

- sum by normalized unit key
- produce merged lines by unit (no carton rounding)
